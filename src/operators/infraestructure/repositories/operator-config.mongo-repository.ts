...
export class OperatorConfigMongoRepository implements OperatorConfigRepository {
    ...
    async findByOperator(operatorId: string): Promise<OperatorConfigEntity | null> {
       return await this.operatorConfigModel.findOne({ operator: operatorId })
    }
    async updateByOperator(operatorId: string, data: Partial<OperatorConfigEntity>): Promise<OperatorConfigEntity | null> {
        return await this.operatorConfigModel.findOneAndUpdate({ operator: operatorId }, data, { new: true })
    }
}