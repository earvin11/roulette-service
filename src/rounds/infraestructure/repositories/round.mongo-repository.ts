...
export class RoundMongoRepository implements RoundRepository {
    ...
    async findManyBy(filter: Record<string, any>, sort?: Record<string, 1 | -1>, limit?: number): Promise<RoundEntity[] | []> {
    ...
    async update(id: string, data: Partial<RoundEntity>): Promise<RoundEntity | null> {
    ...
    async findFilteredRounds(filter: Record<string, any>, page: number, limit: number): Promise<{ rounds: RoundEntity[]; total: number; }> {
        throw new Error('Method not implemented.');
    }
    async findByIdNumber(identifierNumber: string): Promise<RoundEntity[] | []> {
        throw new Error('Method not implemented.');
    }
    async findByIdNumberWithCountBets(identifierNumber: string): Promise<{ round: RoundEntity; totalBets: number; }[] | []> {
        throw new Error('Method not implemented.');
    }
}